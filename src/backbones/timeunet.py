...
class TimeUNet_v1(nn.Module):
    """
    Modified implementation of UTAE for temporo-spatial encoding of satellite image time series.
...
class TimeUNet_v2(nn.Module):
    ...
    Modification is that we apply after few shared convolutions  TAE(classical) which generates
        ...
        which is passed to shared conv layer and then in smallest resolution is again applied TAE(lightweight).
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        for i in range(self.n_stages - 1):
            skip = self.temporal_aggregator(
                feature_maps[-(i + 2)], pad_mask=pad_mask, attn_mask=attn  # they use just attention masks
            )
