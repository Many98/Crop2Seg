...
from src.backbones.tae import LTAE
...
class UTAE(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_encoder = LTAE(in_channels=encoder_widths[-1],  # 128
                                     d_model=d_model,
                                     n_head=n_head,
                                     d_k=d_k,
    ...
    def forward(self, input, batch_positions=None, return_att=False, *args, **kwargs):
        ...
        for i in range(self.n_stages - 1):
            ...
            skip = self.temporal_aggregator(
                feature_maps[-(i + 2)], pad_mask=pad_mask, attn_mask=att  # they use just attention masks
            )
